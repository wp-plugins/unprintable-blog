	...
	$pdf_output = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">
		...
		<body xml:lang="en">
			...
			while (have_posts()) : the_post();
				...
				if(!is_page()) $pdf_output .=  '<p class="small subtitle">' . get_author_name($post->post_author) . ' &middot; ' . date('l, F jS, Y', gcp_pdf_mysql2unix($post->post_date)) . '</p>';
				...
				if(is_single()) {
					$pdf_output .= '<p class="postmetadata alt">
						<span>
							This entry was posted on ' . date('l, F jS, Y', gcp_pdf_mysql2unix($post->post_date)) . ' at ' . date('g:i a', gcp_pdf_mysql2unix($post->post_date)) . ' and is filed under ' . $cat_links . '
